...
var ZuckJS = function () {
    ...
    ZuckJS = window['ZuckitaDaGalera'] = window['Zuck'] = function (timeline, options) {
        var d = document;
        var zuck = this;
        ...
        var zuckModal = function zuckModal() {
            ...
            var createStoryViewer = function createStoryViewer(storyData, className, forcePlay) {
                ...
                var exists = query('#zuck-modal .story-viewer[data-story-id="' + storyId + '"]');
                var currentItemTime = '';
                ...
                each(get(storyData, 'items'), function (i, item) {
                    ...
                    htmlItems += '<div data-time="' + get(item, 'time') + '" data-type="' + get(item, 'type') + '"' + commonAttrs + ' class="item ' + seenClass + ' ' + (currentItem === i ? 'active' : '') + '">' + (get(item, 'type') === 'video' ? '<video class="media" muted webkit-playsinline playsinline preload="auto" src="' + get(item, 'src') + '" ' + get(item, 'type') + '></video><b class="tip muted">' + option('language', 'unmute') + '</b>' : '<div class="media" style="background-image:url(' + get(item, 'src') + ')" ' + get(item, 'type') + '></div>') + (get(item, 'link') ? '<a class="tip link" href="' + get(item, 'link') + '" rel="noopener" target="_blank">' + (linkText == '' || !linkText ? option('language', 'visitLink') : linkText) + '</a>' : '') + '</div>';
                });
            ...
            var createStoryTouchEvents = function createStoryTouchEvents(modalSliderElement) {
                ...
                var touchEnd = function touchEnd(event) {
                    ...
                    if (nextTimer) {
                        ...
                        var _storyViewer = query('#zuck-modal .viewing');
        ...
        zuck.navigateItem = zuck.nextItem = function (direction, event) {
            ...
            var storyViewer = query('#zuck-modal .story-viewer[data-story-id="' + currentStory + '"]');
