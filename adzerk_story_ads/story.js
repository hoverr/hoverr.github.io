...
(function(){
	...
	eventQueue = [],
	divolteJS = 'https://dc01.automatad.com/divolte.js',
	queueIntr = false,
	divolte = undefined,
	...
	createStory = function(container, story){
		...
		createLayer = function(layer){
			...
			switch(layer.type){
				case 'video':
					var video = createElementSetAttr('video', {'preload': 'auto', 'height': layer.height, 'width': layer.width, 'poster': layer.poster, 'muted': '', 'webkit-playsinline': '', 'playsinline': ''}),
						source,
						k = 0;
				...
				case 'image':
					var image = createElementSetAttr('img', {'src': layer.source, 'height': layer.height, 'width': layer.width});
		...
		for(; i < pages.length; i++){
			...
			for(j=0; j<pages[i].layers.length; j++){
				...
				layer = createLayer(pages[i].layers[j]);
	...
	makeItWork = function(storyJson, containers){
		...
		loadVideoOrImage = function(index){
			...
			if(video){
				...
				loaded = function(){
					counter++;
					if(counter == images.length && index == pageIndex) hideLoader();
					else if(index == pageIndex) showLoader();
				};
		...
		setPrevNextStory = function(callback){
			...
			if(prevStoryIndex != -1){
				stories[prevStoryIndex].elm.slider.classList.add('prev');
				stories[prevStoryIndex].elm.slider.classList.remove('current', 'next');
			}
			if(nextStoryIndex != -1){
				stories[nextStoryIndex].elm.slider.classList.add('next');
				stories[nextStoryIndex].elm.slider.classList.remove('current', 'prev');
			}
		...
		setActivePage = function(callback){
			...
			selectedStory.elm.pages[pageIndex].classList.add('active');
			console.log(selectedStory.elm.pages[pageIndex]);
			window.setTimeout(function(){
				for(; k<selectedStory.elm.pages.length; k++){
					...
					if(pageIndex > 0 && k < pageIndex){
						selectedStory.elm.pointer[k].pointer.classList.remove('paused');
		...
		playStory = function(){
			...
			if(selectedStory.uniqueID == storyUniqueId && selectedStory.elm.pages.length > 0){
				...
				pushEventInQueue('widgettap', {'index': storyIndex, 'storyid': storyUniqueId});
			}
		},
		...
		stopAndCloseStory = function(swipe){
			...
			window.setTimeout(function(){
				...
				pushEventInQueue('close', {'type': ((!swipe) ? 'closeclick' : 'swipedown'), 'storyid': storyUniqueId});
		...
		bindTouchEvents = function(elm){
			...
			var touchStart = function(event){
				...
				timer = window.setTimeout(function(){
					selectedStory.elm.pages[pageIndex].classList.add('long-press');
				}, 600);
				...
				if(delta){
					...
					if(!isScrolling){
						if(isValid && !isOutOfBounds){
							if(direction) changeToNextStory();
							else changeToPrevStory();
						}
						else{
					...
					nextTimer = false;
					if(lastTouchOffset.x > window.innerWidth / 3) runNextStory();
					else runPrevStory();
				}
				else resumePlay();
			};
		...
		for(; i<stories.length; i++){
			...
			bindEvent(stories[i].elm.mute, 'click', function(){
				...
				pushEventInQueue('sound', {'status': isMuted ? 'mute' : 'unmute'});
	...
	if(_auto_init){
		_docReady(function(){
			httpGet(apiLink, function(status, data){
				init(JSON.parse(data), false, 'body');
			}, 'application/json');
		});
	}
